  ...
  "relative w-full rounded-lg border px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:translate-y-0.5 [&>svg]:text-current",
  {
    variants: {
      variant: {
        ...
        destructive:
          "text-destructive-foreground [&>svg]:text-destructive-foreground *:data-[slot=alert-description]:text-destructive-foreground/80 border-destructive/50 bg-destructive/10",
        warning:
          "text-amber-900 dark:text-amber-200 [&>svg]:text-amber-600 dark:text-amber-300 border-amber-400/50 bg-amber-50 dark:bg-amber-950/30",
      },
    },
    defaultVariants: {
      variant: "default",
